...
This workspace is not carrying the full upstream `MN755/11Writer` camera and webcam stack.
...
Right now, not wholesale. Upstream still has the bigger webcam runtime and lifecycle stack. Forte has a narrower backend-only camera inventory and ops surface, not the full upstream camera subsystem.
