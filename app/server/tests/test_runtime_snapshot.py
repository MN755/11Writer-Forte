...
from src.services.worker_status_service import mark_worker_started, mark_worker_stopped, publish_worker_heartbeat
...
def seed_runtime_state(client: TestClient, tmp_path: Path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    try:
        worker = mark_worker_started(
            session,
            worker_key="snapshot-source-runtime-worker",
            worker_type="source_runtime_worker",
            actor="snapshot_test",
            metadata_json={"phase": "start"},
            process_token="snapshot-worker-token",
        )
        assert worker.status == "active"
        worker = publish_worker_heartbeat(
            session,
            worker_key="snapshot-source-runtime-worker",
            worker_type="source_runtime_worker",
            actor="snapshot_test",
            process_token="snapshot-worker-token",
            metadata_json={"phase": "heartbeat"},
        )
        assert worker.status == "active"
...
def test_runtime_snapshot_export_and_restore_round_trip(client: TestClient, tmp_path: Path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    assert detail["action"] == "runtime_restore"
    assert detail["replace_existing"] is False
    assert detail["error_type"] == "ValueError"
    assert "empty database" in detail["message"]
    ...
    assert row_counts["worker_statuses"] >= 1
    ...
    assert len(restored_snapshot["worker_statuses"]) >= len(snapshot["worker_statuses"])
    ...
    assert any(row["worker_key"] == "snapshot-source-runtime-worker" for row in restored_snapshot["worker_statuses"])
...
def test_runtime_snapshot_artifact_export_route_writes_snapshot_manifest_and_storage_rows(
    client: TestClient,
    tmp_path: Path,
    monkeypatch,
    ...
    response = client.post(
        "/api/operations/runtime/export-artifacts",
        json={
    ...
    assert payload["output_path"] == str(snapshot_path.resolve())
    assert payload["manifest_path"] == str(manifest_path.resolve())
    assert payload["snapshot_storage_object"]["object_kind"] == "runtime_snapshot_export"
    assert payload["manifest_storage_object"]["object_kind"] == "runtime_snapshot_manifest_export"
    assert payload["snapshot_manifest"]["snapshot_file_name"] == snapshot_path.name
    assert snapshot_path.exists()
    assert manifest_path.exists()

    runtime_rows = client.get(
        "/api/storage/objects",
        params={"owner_type": "runtime_snapshot"},
    )
    assert runtime_rows.status_code == 200
    objects = runtime_rows.json()
    kinds = {row["object_kind"] for row in objects}
    assert "runtime_snapshot_export" in kinds
    assert "runtime_snapshot_manifest_export" in kinds
    restore_default_clickhouse_settings()
