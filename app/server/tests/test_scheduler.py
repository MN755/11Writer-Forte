...
@contextmanager
def scheduler_camera_endpoint_server():
    class Handler(BaseHTTPRequestHandler):
...
def test_runtime_snapshot_export_schedule_writes_snapshot_and_manifest_artifacts(client: TestClient) -> None:
    schedule_response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "scheduled-runtime-snapshot",
            "task_type": "runtime_snapshot_export",
            "interval_seconds": 600,
            "payload_json": {"file_prefix": "scheduled-runtime-snapshot"},
        },
    )
    assert schedule_response.status_code == 200
    task_id = schedule_response.json()["task_id"]
    ...
    assert payload["output_json"]["snapshot_storage_object_id"] is not None
    assert payload["output_json"]["manifest_storage_object_id"] is not None
    assert payload["output_json"]["output_path"].endswith(".json")
    assert payload["output_json"]["manifest_path"].endswith(".manifest.json")

    session = get_session_factory()()
    try:
        snapshot_row = session.get(StorageObjectORM, payload["output_json"]["snapshot_storage_object_id"])
        manifest_row = session.get(StorageObjectORM, payload["output_json"]["manifest_storage_object_id"])
        assert snapshot_row is not None
        assert manifest_row is not None
        assert snapshot_row.object_kind == "runtime_snapshot_export"
        assert manifest_row.object_kind == "runtime_snapshot_manifest_export"
        assert manifest_row.metadata_json["snapshot_storage_object_id"] == snapshot_row.storage_object_id
    finally:
        session.close()


def test_scheduler_run_route_returns_structured_404_for_missing_local_import_input(
    client: TestClient,
    tmp_path: Path,
) -> None:
    missing_fixture = tmp_path / "missing-scheduled-import.json"
    schedule_response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "scheduled-import-missing-file",
            "task_type": "local_import",
            "interval_seconds": 300,
    ...
    payload = run_response.json()
    assert payload["detail"]["task_id"] == task_id
    assert payload["detail"]["task_type"] == "local_import"
    assert payload["detail"]["error_type"] == "FileNotFoundError"
    assert "does not exist" in payload["detail"]["message"]

    session = get_session_factory()()
    try:
        task_run = session.scalar(
            select(ScheduledTaskRunORM)
            .where(ScheduledTaskRunORM.task_id == task_id)
            .order_by(ScheduledTaskRunORM.task_run_id.desc())
            .limit(1)
        )
        assert task_run is not None
        assert task_run.status == "failed"
        assert task_run.task_run_id == payload["detail"]["task_run_id"]
        assert task_run.error_text is not None and "does not exist" in task_run.error_text
    finally:
        session.close()


def test_scheduler_run_route_returns_structured_502_for_source_sync_runtime_failure(client: TestClient) -> None:
    source_response = client.post(
        "/api/sources",
        json={
            "name": "scheduler-route-failing-source",
            "source_kind": "http_json",
            "layer_key": "scheduler-route-source-layer",
            "target_uri": "http://127.0.0.1:1/route-failing.json",
            "metadata_json": {
                "retry_attempts": 1,
    ...
    schedule_response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "scheduler-route-failing-source-sync",
    ...
    payload = run_response.json()
    assert payload["detail"]["task_id"] == task_id
    assert payload["detail"]["task_type"] == "source_sync"
...
def test_run_enabled_schedules_executes_enabled_tasks_even_when_not_due(client: TestClient, tmp_path: Path) -> None:
    fixture = tmp_path / "run-enabled.json"
    fixture.write_text(
        json.dumps(
            [
                {
                    "title": "Run enabled import",
                    "url": "https://enabled.example.com/1",
                    "lat": 30.2,
                    "lon": -95.2,
                }
            ]
    ...
    schedule_response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "run-enabled-import",
            "task_type": "local_import",
            "interval_seconds": 300,
            "target_path": str(fixture),
            "layer_key": "enabled-feed",
        },
    )
    assert schedule_response.status_code == 200
    ...
    enabled_response = client.post("/api/scheduler/run-enabled")
    assert enabled_response.status_code == 200
    assert enabled_response.json()["runs_created"] == 1
...
def test_camera_source_verification_schedule_verifies_candidate_endpoints(
    client: TestClient,
    tmp_path: Path,
) -> None:
    with scheduler_camera_endpoint_server() as base_url:
        fixture = tmp_path / "scheduled-camera-verification.json"
        fixture.write_text(
            json.dumps(
                [
                    {
                        "camera_id": "verify-scheduled-1",
                        "camera_name": "Scheduled Verification Camera",
                        "provider": "MnDOT",
                        "road_name": "I-35W",
                        "status": "active",
        ...
        assert client.post(
            "/api/imports/local",
            json={"source_path": str(fixture), "layer_key": "traffic-camera-verify-scheduled"},
        ).status_code == 200
        assert client.post(
            "/api/cameras/materialize",
            json={"layer_key": "traffic-camera-verify-scheduled", "limit": 10},
        ).status_code == 200

        schedule_response = client.post(
            "/api/scheduler/tasks",
            json={
                "name": "scheduled-camera-source-verification",
                "task_type": "camera_source_verification",
                "interval_seconds": 300,
                "layer_key": "traffic-camera-verify-scheduled",
                "payload_json": {"limit": 10, "timeout_seconds": 2.0},
            },
        )
        assert schedule_response.status_code == 200
        task_id = schedule_response.json()["task_id"]
        ...
        assert payload["status"] == "completed"
        assert payload["records_affected"] == 2
        assert payload["output_json"]["verified_count"] == 2
        assert payload["output_json"]["reachable_count"] == 1
        assert payload["output_json"]["failed_count"] == 1

        camera_sources_response = client.get(
            "/api/camera-sources",
            params={"layer_key": "traffic-camera-verify-scheduled", "limit": 10},
        )
        assert camera_sources_response.status_code == 200
        camera_sources = camera_sources_response.json()
        assert any(row["verification_state"] == "reachable" and row["endpoint_kind"] == "image" for row in camera_sources)
        assert any(row["verification_state"] == "failed" and row["endpoint_kind"] == "page" for row in camera_sources)
        ...
        assert report["verification_task_count"] == 1
        assert report["verification_run_count"] == 1
        assert report["recent_verification_runs"][0]["task_name"] == "scheduled-camera-source-verification"


def test_camera_source_verification_schedule_can_block_private_network_targets(
    client: TestClient,
    tmp_path: Path,
) -> None:
    with scheduler_camera_endpoint_server() as base_url:
        fixture = tmp_path / "scheduled-camera-private-network-block.json"
        fixture.write_text(
            json.dumps(
                [
                    {
                        "camera_id": "verify-scheduled-private-block",
                        "camera_name": "Scheduled Private Block Camera",
                        "provider": "MnDOT",
                        "road_name": "I-35W",
                        "status": "active",
                        "image_url": f"{base_url}/camera.jpg",
        ...
        assert client.post(
            "/api/imports/local",
            json={"source_path": str(fixture), "layer_key": "traffic-camera-verify-private-block"},
        ).status_code == 200
        assert client.post(
            "/api/cameras/materialize",
            json={"layer_key": "traffic-camera-verify-private-block", "limit": 10},
        ).status_code == 200

        schedule_response = client.post(
            "/api/scheduler/tasks",
            json={
                "name": "scheduled-camera-source-private-network-block",
                "task_type": "camera_source_verification",
                "interval_seconds": 300,
                "layer_key": "traffic-camera-verify-private-block",
                "payload_json": {
                    "limit": 10,
                    "timeout_seconds": 2.0,
        ...
        assert payload["status"] == "completed"
        assert payload["records_affected"] == 1
        assert payload["output_json"]["verified_count"] == 1
        assert payload["output_json"]["reachable_count"] == 0
        assert payload["output_json"]["failed_count"] == 1
        assert payload["output_json"]["allow_private_networks"] is False
        assert payload["output_json"]["max_payload_bytes"] == 64


def test_camera_source_verification_schedule_rejects_invalid_payload(client: TestClient) -> None:
    response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "broken-camera-source-verification",
            "task_type": "camera_source_verification",
            "interval_seconds": 300,
            "payload_json": {"endpoint_kind": "nope"},
        },
    )
    assert response.status_code == 409
    assert "Camera source verification" in response.json()["detail"]

    second_response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "broken-camera-source-verification-private-network",
            "task_type": "camera_source_verification",
            "interval_seconds": 300,
            "payload_json": {"allow_private_networks": "nope"},
        },
    )
    assert second_response.status_code == 409
    assert "Camera source verification" in second_response.json()["detail"]


def test_observation_watch_schedule_creates_unscoped_alerts_from_corpus_query(
    client: TestClient,
    tmp_path: Path,
) -> None:
    fixture = tmp_path / "scheduled-observation-watch.json"
    fixture.write_text(
        json.dumps(
            [
                {
    ...
    assert client.post(
        "/api/imports/local",
        json={"source_path": str(fixture), "layer_key": "scheduled-watch-layer"},
    ).status_code == 200

    schedule_response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "scheduled-observation-watch",
            "task_type": "observation_watch_scan",
            "interval_seconds": 300,
            "layer_key": "scheduled-watch-layer",
            "payload_json": {
                "query": "harbor manifest",
                "severity": "warning",
                "limit": 10,
                "candidate_limit": 100,
                "lookback_hours": 72,
    ...
    first_run_response = client.post(f"/api/scheduler/tasks/{task_id}/run")
    assert first_run_response.status_code == 200
    first_payload = first_run_response.json()
    assert first_payload["status"] == "completed"
    assert first_payload["records_affected"] == 1
    assert first_payload["output_json"]["alerts_created"] == 1
    assert first_payload["output_json"]["query_text"] == "harbor manifest"

    second_run_response = client.post(f"/api/scheduler/tasks/{task_id}/run")
    assert second_run_response.status_code == 200
    second_payload = second_run_response.json()
    assert second_payload["records_affected"] == 0
    assert second_payload["output_json"]["alerts_created"] == 0
    ...
    assert alerts[0]["geofence_id"] is None
    assert alerts[0]["event_id"] is None
    assert alerts[0]["trigger_basis_json"]["query_text"] == "harbor manifest"
    assert alerts[0]["trigger_basis_json"]["matched_terms"] == ["harbor", "manifest"]

    report_response = client.get("/api/alerts/report-index")
    assert report_response.status_code == 200
    report = report_response.json()
    assert report["observation_watch_task_count"] == 1
    assert report["observation_watch_run_count"] == 2
    assert report["observation_watch_tasks"][0]["name"] == "scheduled-observation-watch"

    custody_response = client.get("/api/custody/logs")
    assert custody_response.status_code == 200
    assert any(
        row["object_type"] == "observation_watch_scan" and row["action"] == "alerts_evaluated"
        for row in custody_response.json()
    )


def test_observation_watch_schedule_rejects_invalid_payload(client: TestClient) -> None:
    response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "broken-observation-watch",
            "task_type": "observation_watch_scan",
            "interval_seconds": 300,
            "payload_json": {"query": ""},
        },
    ...
    assert response.status_code == 409
    assert "Observation watch" in response.json()["detail"]

    second_response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "broken-observation-watch-limit",
            "task_type": "observation_watch_scan",
            "interval_seconds": 300,
            "payload_json": {"query": "harbor", "candidate_limit": 1, "limit": 5},
        },
    )
    assert second_response.status_code == 409
    assert "Observation watch" in second_response.json()["detail"]
...
def test_scheduler_worker_once_runs_due_tasks(client: TestClient, tmp_path: Path) -> None:
    ...
    assert any(bucket["key"] == "scheduler_worker" for bucket in worker_summary["worker_type_counts"])
    assert any(row["worker_type"] == "scheduler_worker" for row in worker_summary["workers"])
...
def test_source_maintenance_schedule_replays_dead_letters_and_reruns_stale_sources(
    client: TestClient,
    tmp_path: Path,
) -> None:
    stale_fixture = tmp_path / "source-maintenance-stale.json"
    stale_fixture.write_text(
        json.dumps(
            [
                {
                    "title": "Stale source record",
                    "url": "https://maintenance.example.com/stale",
                    "lat": 44.95,
                    "lon": -93.27,
                }
            ]
    ...
    stale_source_response = client.post(
        "/api/sources",
        json={
            "name": "source-maintenance-stale-source",
            "source_kind": "local_file",
            "layer_key": "maintenance-stale-layer",
            "target_uri": str(stale_fixture),
        },
    )
    assert stale_source_response.status_code == 200
    stale_source_id = stale_source_response.json()["source_id"]
    first_run = client.post(f"/api/sources/{stale_source_id}/run")
    assert first_run.status_code == 200

    webhook_source_response = client.post(
        "/api/sources",
        json={
            "name": "source-maintenance-webhook-source",
            "source_kind": "webhook_ingest",
            "layer_key": "maintenance-webhook-layer",
            "target_uri": "webhook://local",
        },
    )
    assert webhook_source_response.status_code == 200
    webhook_source_id = webhook_source_response.json()["source_id"]

    session = get_session_factory()()
    try:
        stale_run = session.scalar(
            select(SourceRunORM)
            .where(SourceRunORM.source_id == stale_source_id)
            .order_by(SourceRunORM.source_run_id.desc())
            .limit(1)
        )
        assert stale_run is not None
        stale_timestamp = scheduler_now() - timedelta(hours=48)
        stale_run.started_at = stale_timestamp
        stale_run.finished_at = stale_timestamp
        checkpoint = session.scalar(
            select(SourceCheckpointORM).where(SourceCheckpointORM.source_id == stale_source_id).limit(1)
        )
        assert checkpoint is not None
        checkpoint.last_success_at = stale_timestamp
        dead_letter = SourceDeadLetterORM(
            ...
            status="pending",
            failure_reason="synthetic scheduler replay test",
            payload_json={
                "title": "Replay me",
                "url": "https://maintenance.example.com/replay",
                "lat": 44.96,
                "lon": -93.28,
            },
            checkpoint_json={},
    ...
    assert run_payload["output_json"]["replayed_dead_letter_count"] == 1
    assert run_payload["output_json"]["rerun_source_count"] == 1
    assert dead_letter_id in run_payload["output_json"]["replayed_dead_letter_ids"]
    assert stale_source_id in run_payload["output_json"]["rerun_source_ids"]

    source_runs = client.get("/api/sources/runs").json()
    stale_runs = [row for row in source_runs if row["source_id"] == stale_source_id]
    assert len(stale_runs) >= 2
    assert stale_runs[0]["status"] in {"completed", "skipped"}

    dead_letters = client.get("/api/sources/dead-letters", params={"source_id": webhook_source_id}).json()
    replayed = next(row for row in dead_letters if row["source_dead_letter_id"] == dead_letter_id)
    assert replayed["status"] == "replayed"
    assert replayed["replay_count"] == 1


def test_source_maintenance_schedule_rejects_invalid_payload(client: TestClient) -> None:
    response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "invalid-source-maintenance-task",
            "task_type": "source_maintenance",
            "interval_seconds": 300,
            "payload_json": {
                "replay_dead_letters": False,
                "run_stale_sources": False,
                "run_failing_sources": False,
            },
        },
    )
    assert response.status_code == 409
    assert "at least one remediation action" in response.json()["detail"]


def test_source_health_scan_schedule_creates_and_closes_recovery_alerts(
    client: TestClient,
    tmp_path: Path,
) -> None:
    stale_fixture = tmp_path / "source-health-stale.json"
    stale_fixture.write_text(
        json.dumps(
            [
                {
                    "title": "Stale health source",
                    "url": "https://health.example.com/stale",
                    "lat": 44.91,
                    "lon": -93.21,
                }
            ]
    ...
    stale_source = client.post(
        "/api/sources",
        json={
            "name": "source-health-stale-source",
            "source_kind": "local_file",
            "layer_key": "source-health-layer",
            "target_uri": str(stale_fixture),
        },
    )
    assert stale_source.status_code == 200
    stale_source_id = stale_source.json()["source_id"]
    assert client.post(f"/api/sources/{stale_source_id}/run").status_code == 200

    failing_source = client.post(
        "/api/sources",
        json={
            "name": "source-health-failing-source",
            "source_kind": "http_json",
            "layer_key": "source-health-layer",
            "target_uri": "http://127.0.0.1:1/source-health.json",
            "metadata_json": {
                "retry_attempts": 1,
                "request_timeout_seconds": 1,
            },
    ...
    webhook_source = client.post(
        "/api/sources",
        json={
            "name": "source-health-webhook-source",
            "source_kind": "webhook_ingest",
            "layer_key": "source-health-layer",
            "target_uri": "webhook://local",
        },
    )
    assert webhook_source.status_code == 200
    webhook_source_id = webhook_source.json()["source_id"]
    ...
    try:
        try:
            ...
            raise AssertionError("Expected failing source run to fail.")
        ...
        dead_letter = SourceDeadLetterORM(
            ...
            stage="webhook_ingest",
            status="pending",
            failure_reason="synthetic health alert test",
            payload_json={
                "title": "Source health replay",
                "url": "https://health.example.com/replay",
                "lat": 44.92,
                "lon": -93.22,
            },
            checkpoint_json={},
    ...
    assert first_payload["output_json"]["created_alert_count"] == 3
    assert first_payload["output_json"]["closed_alert_count"] == 0

    alerts = client.get("/api/alerts", params={"limit": 20}).json()
    source_health_alerts = [row for row in alerts if (row["dedupe_key"] or "").startswith("source-health:")]
    assert len(source_health_alerts) == 3
    assert any(row["trigger_basis_json"]["condition"] == "stale" for row in source_health_alerts)
    assert any(row["trigger_basis_json"]["condition"] == "failed" for row in source_health_alerts)
    assert any(row["trigger_basis_json"]["condition"] == "dead_letters" for row in source_health_alerts)

    assert client.post(f"/api/sources/{stale_source_id}/run").status_code == 200
    assert client.post(f"/api/sources/dead-letters/{dead_letter_id}/replay").status_code == 200
    assert client.patch(
        f"/api/sources/{failing_source_id}",
        json={"enabled": False},
    ).status_code == 200
    ...
    assert second_payload["output_json"]["created_alert_count"] == 0
    assert second_payload["output_json"]["closed_alert_count"] == 3

    alerts_after = client.get("/api/alerts", params={"limit": 20}).json()
    source_health_alerts_after = [row for row in alerts_after if (row["dedupe_key"] or "").startswith("source-health:")]
    assert len(source_health_alerts_after) == 3
    assert all(row["status"] == "closed" for row in source_health_alerts_after)


def test_source_health_scan_schedule_rejects_invalid_payload(client: TestClient) -> None:
    response = client.post(
        "/api/scheduler/tasks",
        json={
            "name": "invalid-source-health-scan-task",
            "task_type": "source_health_scan",
            "interval_seconds": 300,
            "payload_json": {
                "alert_on_stale": False,
                "alert_on_failed": False,
                "alert_on_dead_letters": False,
            },
        },
    )
    assert response.status_code == 409
    assert "at least one alert condition" in response.json()["detail"]
