...
def test_event_export_bundle_includes_evidence_products_and_runs(
    ...
    event_alerts = client.get("/api/alerts", params={"event_id": event_id})
    assert event_alerts.status_code == 200
    assert len(event_alerts.json()) == 2
...
def test_event_export_bundle_rejects_lower_redaction_level_than_event(
    ...
    assert detail["action"] == "export_event_bundle"
    assert detail["event_id"] == event_id
    assert detail["max_redaction_level"] == "public"
    assert detail["error_type"] == "ValueError"
    assert "cannot be exported" in detail["message"]
