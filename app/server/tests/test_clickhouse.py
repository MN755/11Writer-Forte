...
def test_clickhouse_diagnostics_support_local_only_mode(client: TestClient, monkeypatch) -> None:
    ...
    assert payload["r2_configured"] is False
    assert payload["warnings"] == []
    assert any("local-only mode" in note for note in payload["notes"])
...
def test_clickhouse_rehydrate_route_returns_structured_error_when_r2_is_unconfigured(
    client: TestClient,
) -> None:
    response = client.post(
        "/api/operations/clickhouse/rehydrate",
        params={"archive_glob_url": "https://acct.r2.cloudflarestorage.com/example/path/*.parquet"},
    )
    assert response.status_code == 409
    detail = response.json()["detail"]
    assert detail["action"] == "clickhouse_rehydrate"
    assert detail["archive_glob_url"].endswith("*.parquet")
