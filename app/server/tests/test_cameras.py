...
def test_camera_inventory_materialization_and_update(client: TestClient, tmp_path: Path) -> None:
    ...
    assert any(
        camera["external_id"] == "mndot-i35w-001" and camera["active"] is False
    ...
    assert (
        ops_payload["latest_observation"]["observation_id"]
    ...
    assert (
        report_index["recent_materializations"][0]["action"] == "camera_materialization_completed"
    )
    ...
    assert (
        source_ops["latest_observation"]["observation_id"]
        == second_payload["cameras"][0]["observation_id"]
    )
    ...
    assert any(
        row["object_type"] == "camera_inventory" and row["action"] == "camera_registered"
        for row in custody_rows
    )
    assert any(
        row["object_type"] == "camera_inventory" and row["action"] == "camera_updated"
        for row in custody_rows
    )
    assert any(
