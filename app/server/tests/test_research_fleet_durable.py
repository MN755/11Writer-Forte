...
def _provider() -> ResearchProviderORM:
    return ResearchProviderORM(
        ...
        languages_json=["en"],
        capabilities_json=["structured_api", "document"],
        default_budget_json={"requests": 10, "bytes": 5000000},
...
def test_durable_provider_run_persists_lease_budget_and_idempotency() -> None:
    ...
    with Session(engine) as session:
        ...
        assert stored.provider.provider_key == "federal_register_api"
    ...
    with Session(engine) as session:
        ...
        session.add(
            ResearchProviderRunORM(
                ...
                response_hash="b" * 64,
                coverage_gaps_json={"provider:federal_register_api": {"status": "open"}},
            )
        )
        session.commit()
        ...
        assert snapshot["research_providers"][0]["provider_key"] == "federal_register_api"
        ...
        assert session.scalar(select(ResearchProviderORM).where(ResearchProviderORM.provider_key == "federal_register_api"))
