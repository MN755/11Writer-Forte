...
def test_operations_report_summarizes_runtime_activity(client: TestClient, tmp_path: Path) -> None:
    ...
    assert payload["runtime_readiness"]["overall_status"] in {"not_ready", "degraded", "ready"}
    ...
    assert payload["worker_status_summary"]["total_count"] == 0
    ...
    assert payload["event_inventory_summary"]["total_count"] == 0
    assert payload["entity_inventory_summary"]["total_count"] == 0
    ...
    export_response = client.post(
        "/api/operations/report/export",
        json={"output_path": str(export_path), "hours": 24.0, "limit": 5},
    )
    assert export_response.status_code == 200
    export_payload = export_response.json()
    assert export_payload["output_path"] == str(export_path.resolve())
    assert export_payload["storage_object"]["object_kind"] == "operations_report_export"
    assert export_payload["storage_object"]["owner_type"] == "operations_report"
    ...
    assert operations_rows.status_code == 200
    operations_objects = operations_rows.json()
    assert len(operations_objects) == 1
    assert operations_objects[0]["metadata_json"]["hours"] == 24.0
    ...
    assert camera_payload["worker_status_summary"]["total_count"] == 0
    ...
    assert camera_payload["event_inventory_summary"]["total_count"] == 0
    assert camera_payload["entity_inventory_summary"]["total_count"] == 0
