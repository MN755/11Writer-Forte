...
@app.command("create-investigation")
def create_investigation_command(
    slug: str,
    question: str,
    operator_scope_json: str = "{}",
    time_window_json: str = "{}",
    geography_json: str = "{}",
    source_policy_snapshot_json: str = "{}",
    research_budget_json: str = "{}",
    evidence_threshold_json: str = "{}",
) -> None:
    """Create a durable, public-information investigation record."""
    payload = InvestigationCreate(
        slug=slug,
        question=question,
        operator_scope_json=parse_json_object_option(operator_scope_json, "--operator-scope-json")
        or {},
        time_window_json=parse_json_object_option(time_window_json, "--time-window-json") or {},
        geography_json=parse_json_object_option(geography_json, "--geography-json") or {},
        source_policy_snapshot_json=(
            parse_json_object_option(source_policy_snapshot_json, "--source-policy-snapshot-json")
            or {}
        ),
        research_budget_json=parse_json_object_option(
            research_budget_json, "--research-budget-json"
        )
        or {},
        evidence_threshold_json=(
            parse_json_object_option(evidence_threshold_json, "--evidence-threshold-json") or {}
        ),
    )
    init_db()
    session = get_session_factory()()
    try:
        print_banner()
        echo_model_json(InvestigationRead, create_investigation(session, payload, actor="cli"))
    except ValueError as exc:
        raise typer.BadParameter(str(exc)) from exc
    finally:
        session.close()
...
def show_entity_profile_command(entity_id: int, max_redaction_level: str = "public") -> None:
    ...
    try:
        ...
        typer.echo(
            json.dumps(
                get_entity_profile(session, entity_id, max_redaction_level=max_redaction_level),
                default=str,
                indent=2,
            )
        )
...
@app.command("list-investigations")
def list_investigations_command(state: str | None = None, limit: int = 200) -> None:
    """List investigation records without triggering research work."""
    init_db()
    session = get_session_factory()()
    try:
        print_banner()
        adapter = TypeAdapter(list[InvestigationRead])
        rows = adapter.dump_python(
            adapter.validate_python(list_investigations(session, state=state, limit=limit)),
            mode="json",
        )
...
@app.command("transition-investigation")
def transition_investigation_command(
    investigation_id: int,
    state: str,
    stop_reason: str | None = None,
) -> None:
    """Move an investigation through the explicit lifecycle."""
    init_db()
    session = get_session_factory()()
    try:
        print_banner()
        echo_model_json(
            ...
            transition_investigation(
                session,
                investigation_id,
                InvestigationTransitionRequest(state=state, stop_reason=stop_reason),
                actor="cli",
            ),
        )
    except ValueError as exc:
...
@app.command("archive-investigation")
def archive_investigation_command(investigation_id: int, stop_reason: str | None = None) -> None:
    """Archive while retaining reports and promoted evidence."""
    init_db()
    session = get_session_factory()()
    try:
        print_banner()
