...
class WorkerStatusORM(TimestampMixin, Base):
    __tablename__ = "worker_statuses"

    worker_status_id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    worker_key: Mapped[str] = mapped_column(String(160), unique=True, index=True)
    worker_type: Mapped[str] = mapped_column(String(60), index=True)
    actor: Mapped[str] = mapped_column(String(120), index=True)
    status: Mapped[str] = mapped_column(String(30), default="idle", index=True)
    process_token: Mapped[str | None] = mapped_column(String(120), default=None, index=True)
    last_started_at: Mapped[datetime | None] = mapped_column(default=None)
    last_seen_at: Mapped[datetime | None] = mapped_column(default=None, index=True)
    last_iteration_at: Mapped[datetime | None] = mapped_column(default=None)
    last_stopped_at: Mapped[datetime | None] = mapped_column(default=None)
