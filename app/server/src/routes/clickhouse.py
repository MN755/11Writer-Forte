...
def provision_clickhouse(session: Session = Depends(get_db)) -> dict[str, object]:
    try:
        ...
        raise translate_service_error(exc, action="clickhouse_provision") from exc
...
def sync_clickhouse(
    ...
    try:
        ...
        raise translate_service_error(
            exc,
            action="clickhouse_sync",
            context={"layer_key": layer_key, "source_domain": source_domain},
        ) from exc
    except RuntimeError as exc:
        raise translate_service_error(
            exc,
...
def clickhouse_r2_config() -> dict[str, object]:
    try:
        ...
        raise translate_service_error(exc, action="clickhouse_r2_config") from exc
...
def rehydrate_clickhouse(
    ...
    try:
        ...
        raise translate_service_error(
            exc,
            action="clickhouse_rehydrate",
            context={"archive_glob_url": archive_glob_url},
        ) from exc
    except RuntimeError as exc:
        raise translate_service_error(
            exc,
