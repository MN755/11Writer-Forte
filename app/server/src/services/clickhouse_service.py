...
def build_clickhouse_diagnostics() -> dict[str, object]:
    ...
    if not settings.clickhouse_r2_configured:
        if settings.clickhouse_r2_storage_mode in {"hybrid", "r2_disk"}:
            warnings.append("ClickHouse remote archive/storage mode is selected, but Cloudflare R2 settings are incomplete.")
        else:
            notes.append("ClickHouse is running in local-only mode; remote archive and rehydration paths are disabled.")
...
def query_observations_for_clickhouse(
    ...
    *,
    ...
    statement = select(ObservationORM).order_by(
        ObservationORM.observed_at.desc().nullslast(),
        ObservationORM.updated_at.desc(),
    )
    if layer_key:
...
def deserialize_clickhouse_observation_row(row: dict[str, Any]) -> ObservationQueryRecord:
    ...
    return ObservationQueryRecord(
        ...
        observed_at=parse_clickhouse_datetime_nullable(row.get("observed_at")),
...
def parse_clickhouse_datetime_nullable(value: object) -> datetime | None:
    if value is None or value == "":
        return None
    return parse_clickhouse_datetime(value)
