...
def build_database_diagnostics(session: Session) -> dict[str, object]:
    ...
    try:
        ...
        if not migration_status.version_table_present:
            warnings.append("Runtime schema revision state is missing.")
        elif not migration_status.schema_up_to_date:
            warnings.append("Runtime schema revision is behind the current head.")
    except Exception:
    ...
    return {
        ...
        "migration": {
            "current_revision": migration_status.current_revision if migration_status is not None else None,
            "head_revision": migration_status.head_revision if migration_status is not None else "unknown",
            "version_table_present": migration_status.version_table_present if migration_status is not None else False,
            "has_application_tables": migration_status.has_application_tables if migration_status is not None else False,
            "schema_up_to_date": migration_status.schema_up_to_date if migration_status is not None else False,
        },
