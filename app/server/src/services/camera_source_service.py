...
from src.services.source_service import (
    ...
    build_fetch_runtime_context,
    enforce_fetch_target_network_policy,
    merge_fetch_runtime_metadata,
)
...
@dataclass(frozen=True)
class CameraSourceVerificationOutcome:
    verification_state: str
...
def verify_camera_source_inventory(
    session: Session,
    *,
    ...
    actor: str = "camera_source_verifier",
) -> dict[str, object]:
    fetch_config = build_camera_source_fetch_config(
    ...
    sources = resolve_camera_sources_for_verification(
        session,
    ...
    for source in sources:
        outcome = verify_camera_endpoint(
            source.endpoint_url,
            endpoint_kind=source.endpoint_kind,
            fetch_config=fetch_config,
            fetch_runtime_context=fetch_runtime_context,
        )
        if outcome.verification_state == "reachable":
            reachable_count += 1
        else:
            failed_count += 1
        changes = apply_camera_source_verification(source, outcome)
        verified_ids.append(source.camera_source_inventory_id)
        session.add(
            CustodyLogORM(
                object_type="camera_source_inventory",
                object_id=str(source.camera_source_inventory_id),
                action="camera_source_verified",
                actor=actor,
                details_json={
    ...
    session.add(
        CustodyLogORM(
            object_type="camera_source_verification",
            object_id=(
                str(camera_source_inventory_id)
                if camera_source_inventory_id is not None
                else f"{layer_key or 'all'}:{source_domain or 'all'}:{endpoint_kind or 'all'}"
            ),
            action="camera_source_verification_completed",
            actor=actor,
            details_json={
                "camera_source_inventory_id": camera_source_inventory_id,
                "layer_key": layer_key,
                ...
                "min_request_interval_seconds": fetch_config.min_request_interval_seconds,
                "verified_count": len(sources),
                "reachable_count": reachable_count,
                "failed_count": failed_count,
                "camera_source_inventory_ids": verified_ids,
                **merge_fetch_runtime_metadata(fetch_runtime_context, fetch_config),
            },
        )
    )
    session.commit()
    ...
    verified_sources = (
        list(
            session.scalars(
                select(CameraSourceInventoryORM)
                .where(CameraSourceInventoryORM.camera_source_inventory_id.in_(verified_ids))
                .order_by(CameraSourceInventoryORM.camera_source_inventory_id.asc())
            )
        )
        if verified_ids
...
def build_camera_source_fetch_config(
    *,
    ...
    effective_max_payload_bytes = max(
        1,
        int(
            settings.source_fetch_max_payload_bytes
            if max_payload_bytes is None
            else max_payload_bytes
        ),
    )
    effective_allow_private_networks = (
        settings.source_fetch_allow_private_networks
        if allow_private_networks is None
        else allow_private_networks
    )
    effective_min_request_interval_seconds = max(
        0.0,
        float(
    ...
    return SourceFetchConfig(
        ...
        headers={"User-Agent": "11Writer-Forte/0.1 (+camera-source-verifier)"},
        max_payload_bytes=effective_max_payload_bytes,
        allow_private_networks=effective_allow_private_networks,
        min_request_interval_seconds=effective_min_request_interval_seconds,
    )
...
def build_camera_source_inventory_summary(
    ...
    *,
    ...
    return {
        ...
        "verification_state_counts": build_camera_source_summary_buckets(
            sources,
            lambda source: source.verification_state,
        ),
    }
...
def build_camera_source_ops_report_index(
    ...
    *,
    ...
    verification_tasks = list(
        session.scalars(
            select(ScheduledTaskORM)
            .where(ScheduledTaskORM.task_type == "camera_source_verification")
            .order_by(ScheduledTaskORM.task_id.asc())
        )
    )
    matching_verification_tasks = [
        task
        for task in verification_tasks
        if refresh_task_matches_scope(task, layer_key=layer_key, source_domain=source_domain)
        and (
            endpoint_kind is None
            or (
                isinstance(task.payload_json, dict)
                and task.payload_json.get("endpoint_kind") == endpoint_kind
            )
            or (
                isinstance(task.payload_json, dict)
                and not task.payload_json.get("endpoint_kind")
            )
        )
    ]
    verification_task_lookup = {task.task_id: task for task in matching_verification_tasks}
    verification_task_ids = list(verification_task_lookup)
    all_verification_runs: list[ScheduledTaskRunORM] = []
    recent_verification_runs: list[dict[str, object]] = []
    if verification_task_ids:
        all_verification_runs = list(
            session.scalars(
                select(ScheduledTaskRunORM)
                .where(ScheduledTaskRunORM.task_id.in_(verification_task_ids))
                .order_by(ScheduledTaskRunORM.task_run_id.desc())
            )
        )
        recent_verification_runs = [
            serialize_refresh_run(verification_task_lookup[run.task_id], run)
            for run in all_verification_runs[:limit]
            if run.task_id in verification_task_lookup
        ]
    ...
    verification_logs = list(
        session.scalars(
            select(CustodyLogORM)
            .where(CustodyLogORM.object_type == "camera_source_verification")
            .order_by(CustodyLogORM.created_at.desc())
            .limit(max(limit * 5, 50))
        )
    )
    ...
    return {
        ...
        "verification_task_count": len(matching_verification_tasks),
        "verification_run_count": len(all_verification_runs),
        "verification_failure_count": sum(1 for run in all_verification_runs if run.status == "failed"),
        "verification_tasks": matching_verification_tasks,
        "recent_verification_runs": recent_verification_runs,
...
def build_camera_source_candidates(camera: CameraInventoryORM) -> list[CameraSourceCandidate]:
    ...
    for endpoint_kind, endpoint_url in endpoints:
        if not endpoint_url:
            continue
        base_score = compute_camera_source_score(camera, endpoint_kind, checked_at)
        candidates.append(
...
def apply_camera_source_candidate(
    ...
    effective_verification_state = resolve_materialized_verification_state(
        current_state=record.verification_state,
        candidate_state=candidate.verification_state,
    )
    effective_last_checked_at = (
        record.last_checked_at
        if effective_verification_state != candidate.verification_state and record.last_checked_at is not None
        else candidate.last_checked_at
    )
    effective_score = apply_verification_adjustment(candidate.graduation_score, effective_verification_state)
    effective_status = derive_camera_source_status_for_state(effective_score, effective_verification_state)
    field_mapping = {
        "camera_inventory_id": candidate.camera_inventory_id,
        "observation_id": candidate.observation_id,
        "external_id": candidate.external_id,
        ...
        "active": candidate.active,
        "last_observed_at": candidate.last_observed_at,
        "last_checked_at": effective_last_checked_at,
        "confidence_score": candidate.confidence_score,
        "graduation_score": effective_score,
        "metadata_json": merge_metadata(
            record.metadata_json,
            {
                **candidate.metadata_json,
                "base_graduation_score": candidate.graduation_score,
            },
        ),
    }
...
def resolve_materialized_verification_state(*, current_state: str, candidate_state: str) -> str:
    if candidate_state != "observed":
        return candidate_state
    if current_state in {"reachable", "failed"}:
        return current_state
    return candidate_state


def apply_verification_adjustment(base_score: float, verification_state: str) -> float:
    if verification_state == "reachable":
        adjusted = base_score + 0.15
...
def derive_camera_source_status_for_state(score: float, verification_state: str) -> str:
    if verification_state == "failed":
        return "review" if score >= 0.45 else "candidate"
    return derive_camera_source_status(score)
...
def resolve_camera_sources_for_verification(
    session: Session,
    *,
    camera_source_inventory_id: int | None,
    layer_key: str | None,
    source_domain: str | None,
...
def verify_camera_endpoint(
    ...
    *,
    ...
    try:
        ...
        return CameraSourceVerificationOutcome(
    ...
    for method in ("HEAD", "GET"):
        ...
        if method == "GET" and endpoint_kind != "page":
            request_headers["Range"] = "bytes=0-0"
        request = Request(endpoint_url, headers=request_headers, method=method)
        try:
            apply_fetch_request_pacing(
                endpoint_url,
                fetch_config,
            ...
            with urlopen(request, timeout=fetch_config.timeout_seconds) as response:
                ...
                final_url = getattr(response, "url", endpoint_url)
                if method == "GET":
                    payload = response.read(fetch_config.max_payload_bytes + 1)
                    if len(payload) > fetch_config.max_payload_bytes:
                        raise RuntimeError(
                            f"Camera source verification payload exceeded max_payload_bytes={fetch_config.max_payload_bytes}."
                        )
                reachable = endpoint_response_matches_kind(
                    endpoint_url=endpoint_url,
                    final_url=final_url,
                    endpoint_kind=endpoint_kind,
                ...
                return CameraSourceVerificationOutcome(
                    verification_state="reachable" if reachable else "failed",
                    status_code=int(status_code),
                    content_type=content_type,
                    final_url=final_url,
                    checked_at=checked_at,
                    error_text=None if reachable else f"Unexpected content type for {endpoint_kind} endpoint.",
                )
    ...
    return CameraSourceVerificationOutcome(
        ...
        final_url=endpoint_url,
        checked_at=checked_at,
        error_text=str(last_error) if last_error is not None else "Verification request failed.",
    )


def endpoint_response_matches_kind(
    *,
    endpoint_url: str,
    final_url: str,
    endpoint_kind: str,
    ...
    if endpoint_kind == "stream":
        return (
            ...
            or lower_url.endswith(".m3u8")
        )
    if endpoint_kind == "page":
        return normalized_type.startswith("text/html") or lower_url.endswith((".html", ".htm"))
    return False


def apply_camera_source_verification(
    record: CameraSourceInventoryORM,
    outcome: CameraSourceVerificationOutcome,
) -> dict[str, dict[str, object]]:
    changes: dict[str, dict[str, object]] = {}
    base_score = extract_base_graduation_score(record)
    adjusted_score = apply_verification_adjustment(base_score, outcome.verification_state)
    adjusted_status = derive_camera_source_status_for_state(adjusted_score, outcome.verification_state)
    merged_metadata = merge_metadata(
        record.metadata_json,
        {
            ...
            "verification": {
                ...
                "error_text": outcome.error_text,
                "endpoint_host": urlparse(record.endpoint_url).netloc or None,
            },
        },
    )
    field_mapping = {
        ...
        "graduation_score": adjusted_score,
...
def extract_base_graduation_score(record: CameraSourceInventoryORM) -> float:
    metadata = record.metadata_json if isinstance(record.metadata_json, dict) else {}
    value = metadata.get("base_graduation_score")
    ...
    if isinstance(value, str):
        try:
            ...
            pass
    return round(max(0.0, min(float(record.graduation_score), 1.0)), 3)
