...
def materialize_fused_events(
    ...
    for summary in summaries:
        ...
        if event is None:
            event = EventORM(
                slug=slug,
                title=build_event_title(summary, related_entities, provisional_mentions),
                ...
                metadata_json=build_event_metadata(
                    summary,
                    linked_observations,
                    related_entities,
                    provisional_mentions,
            ...
            event.title = build_event_title(summary, related_entities, provisional_mentions)
            event.summary = build_event_summary(
                summary,
                linked_observations,
                related_entities,
                provisional_mentions,
        ...
        related_alerts = resolve_related_alerts(session, event=event, observations=linked_observations)
        attach_alerts_to_event(session, event=event, alerts=related_alerts, actor=actor)
        event.summary = build_event_summary(
            summary,
            linked_observations,
            related_entities,
...
def build_event_title(
    ...
    if related_entities:
        return (
            f"Fused Event: {related_entities[0]['canonical_name']} near "
            f"{round(float(centroid[1]), 3)}, {round(float(centroid[0]), 3)}"
        )
    if provisional_mentions:
        return (
            f"Fused Event: {provisional_mentions[0]['canonical_name']} near "
            f"{round(float(centroid[1]), 3)}, {round(float(centroid[0]), 3)}"
        )
    return (
...
def build_event_summary(
    ...
    alert_phrase = summarize_alert_phrase(related_alerts)
    assessment = build_event_analytic_assessment(summary, observations, related_entities, provisional_mentions)
    return (
        ...
        f"Layers: {layer_phrase}. Sources: {citations}. {entity_phrase} {alert_phrase} "
        f"Primary confidence drivers: {' '.join(assessment['drivers'][:2]) if assessment['drivers'] else 'Limited corroboration.'} "
        f"Primary cautions: {' '.join(assessment['cautions'][:1]) if assessment['cautions'] else 'No major weakening factors detected.'}"
    )
...
def ensure_situation_products(
    ...
    cited_summary = build_cited_summary(
        event,
        observations,
        summary,
        related_entities,
...
def build_cited_summary(
    ...
    if related_alerts:
        lines.append(summarize_alert_phrase(related_alerts))
    if assessment["drivers"]:
        lines.append("Confidence drivers: " + " ".join(assessment["drivers"]))
    if assessment["cautions"]:
        lines.append("Analytic cautions: " + " ".join(assessment["cautions"]))
    return "\n\n".join(lines)
...
def build_detailed_report(
    ...
    related_alert_lines = build_related_alert_lines(related_alerts)
    ...
    lines = [
        ...
        f"- Integrity sources: {summary['integrity_source_count']}",
        f"- Ground-truth hits: {summary['ground_truth_count']}",
        f"- Linked entities: {len(related_entities)}",
        f"- Related alerts: {len(related_alerts)}",
        "",
        "Confidence Drivers",
    ]
    lines.extend([f"- {item}" for item in assessment["drivers"]] or ["- Limited positive drivers beyond baseline corroboration."])
    lines.extend([
        "",
        "Weakening Factors",
    ])
    lines.extend([f"- {item}" for item in assessment["cautions"]] or ["- No major weakening factors detected in the current evidence set."])
    lines.extend([
        "",
    ...
    lines.extend(related_alert_lines)
    ...
    for observation in sorted(observations, key=observation_timeline_key):
        observed_at = extract_observation_timestamp(observation) or normalize_timestamp(observation.created_at)
        lines.append(
        ...
        rows.append(
            {
                ...
                "signal_conflict_count": (
                    int(entity.metadata_json.get("signal_conflict_count", 0))
                    if isinstance(entity.metadata_json, dict)
                    else 0
                ),
                "signal_conflicts": (
                    entity.metadata_json.get("signal_conflicts", [])
                    if isinstance(entity.metadata_json, dict)
                    else []
                ),
                "evidence_strength": (
                    entity.metadata_json.get("evidence_strength")
                    if isinstance(entity.metadata_json, dict)
                    else None
                ),
            }
...
def build_event_metadata(
    ...
    alert_status_counts = Counter(alert.status for alert in related_alerts)
    alert_severity_counts = Counter(alert.severity for alert in related_alerts)
    assessment = build_event_analytic_assessment(summary, observations, related_entities, provisional_mentions)
    return {
        "fusion_cluster_id": summary["cluster_id"],
        "verification_score": summary["verification_score"],
        ...
        "linked_entity_conflict_count": sum(int(row.get("signal_conflict_count", 0)) for row in related_entities),
        "linked_entity_strengths": sorted(
            {
        ...
        "open_alert_count": int(alert_status_counts.get("open", 0)),
        "alert_status_counts": [
            {"key": key, "count": count}
            for key, count in sorted(alert_status_counts.items(), key=lambda item: (-item[1], item[0]))
        ],
        "alert_severity_counts": [
            {"key": key, "count": count}
            for key, count in sorted(alert_severity_counts.items(), key=lambda item: (-item[1], item[0]))
        ],
...
def summarize_alert_phrase(related_alerts: list[AlertORM]) -> str:
    if not related_alerts:
        return ""
    open_count = sum(1 for alert in related_alerts if alert.status == "open")
    severity_counts = Counter(alert.severity for alert in related_alerts)
    top_severity = sorted(severity_counts.items(), key=lambda item: (-item[1], item[0]))[0][0]
    return (
        f"Related operational alerts: {len(related_alerts)} total, {open_count} open, "
        f"highest observed severity bucket led by {top_severity}."
    )
    ...
    if related_entities:
        return [
            (
                ...
                f"signal_conflicts={int(row.get('signal_conflict_count', 0))}"
            )
            for row in related_entities
        ]
    if provisional_mentions:
...
def build_related_alert_lines(related_alerts: list[AlertORM]) -> list[str]:
    if not related_alerts:
        return ["- No related alert records are currently linked to this event."]
    return [
        (
            f"- Alert {alert.alert_id}: severity={alert.severity}, status={alert.status}, "
            f"geofence_id={alert.geofence_id or 'none'}, message={alert.message}"
        )
...
def resolve_related_alerts(
    ...
    *,
    ...
    observation_ids = {observation.observation_id for observation in observations}
    alerts = list(session.scalars(select(AlertORM).order_by(AlertORM.created_at.asc(), AlertORM.alert_id.asc())))
    related: list[AlertORM] = []
    for alert in alerts:
        trigger_basis = alert.trigger_basis_json if isinstance(alert.trigger_basis_json, dict) else {}
        trigger_observation_id = trigger_basis.get("observation_id")
        if alert.event_id == event.event_id:
            related.append(alert)
            continue
        if trigger_observation_id in observation_ids:
            related.append(alert)
    return related


def attach_alerts_to_event(
    session: Session,
    *,
    event: EventORM,
...
def build_event_analytic_assessment(
    summary: dict[str, object],
    observations: list[ObservationORM],
    related_entities: list[dict[str, object]],
    provisional_mentions: list[dict[str, object]],
    ...
    drivers: list[str] = []
    cautions: list[str] = []
    verification_score = float(summary["verification_score"])
    time_span_minutes = int(summary["time_span_minutes"])
    source_domain_count = int(summary["source_domain_count"])
    layer_count = int(summary["layer_count"])
    integrity_source_count = int(summary["integrity_source_count"])
    ground_truth_count = int(summary["ground_truth_count"])
    trusted_observation_count = int(summary["trusted_observation_count"])

    if verification_score >= 0.9:
        drivers.append(f"High composite verification score at {verification_score:.2f}.")
    if source_domain_count >= 2:
        drivers.append(f"Independent corroboration spans {source_domain_count} source domains.")
    if layer_count >= 2:
        drivers.append(f"Evidence spans {layer_count} data layers.")
    if integrity_source_count > 0:
        drivers.append(f"Integrity-source participation count: {integrity_source_count}.")
    if ground_truth_count > 0:
        drivers.append(f"Ground-truth observations present: {ground_truth_count}.")
    if related_entities:
        drivers.append(f"Resolved linked entities present: {len(related_entities)}.")
    elif provisional_mentions:
        drivers.append(f"Repeated unresolved entity mentions present: {len(provisional_mentions)}.")
    linked_entity_conflict_count = sum(int(row.get("signal_conflict_count", 0)) for row in related_entities)

    if integrity_source_count == 0:
        cautions.append("No integrity source appears in the corroborating evidence set.")
    if ground_truth_count == 0:
        cautions.append("No ground-truth observation directly confirms the event.")
    if source_domain_count <= 1:
        cautions.append("Source-domain diversity is limited.")
    if trusted_observation_count == 0:
        cautions.append("No trusted observations contribute to the cluster.")
    if time_span_minutes > 360:
        cautions.append(f"Event cluster spans a broad temporal window of {time_span_minutes} minutes.")
    if verification_score < 0.75:
        cautions.append(f"Composite verification score remains modest at {verification_score:.2f}.")
    if linked_entity_conflict_count > 0:
        cautions.append(
            f"Linked entities carry {linked_entity_conflict_count} conflicting identifier or naming signal groups."
        )
    if not cautions:
        cautions.append("No major weakening factors were detected in the current event evidence set.")
    ...
    return {
        ...
        "linked_entity_count": len(related_entities),
        "linked_entity_conflict_count": linked_entity_conflict_count,
    }
