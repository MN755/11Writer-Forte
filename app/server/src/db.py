...
from src.migrations import DatabaseRevisionStatus, upgrade_database
...
def init_db(*, auto_upgrade: bool | None = None) -> DatabaseRevisionStatus:
    ...
    settings = get_settings()
    if auto_upgrade or (auto_upgrade is None and settings.database_auto_migrate):
        return upgrade_database(engine)
    return upgrade_database(engine, revision="head")
